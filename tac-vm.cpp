        ...
        if(tac[i].size() > 1){
            if(tac[i].size() == 6 and tac[i][1] == "=" and isOperator(tac[i][3])){
                ...
                if(tac[i][0] == "GOTO"){
                    ...
                    if(tac[i][0]=="append:"){is_append=true;
                    }
                    ins.pop_back();
                    ins += " " + tac[i][1];
                    vm.push_back(ins);
                ...
                if(tac[i][1] == "arg"){
                    ...
                    if(is_append) {
                        if(tac[i][3]=="STRING") {
                            is_append_ID=tac[i][4];
                        }
                    }
                }
                else{
                    // simple assign : t0 = t1 INT
                    if(tac[i][3] == "STR" || tac[i][3]=="STRING"){
                        if(tac[i][2][0]=='@'){
                        pair<pair<string, string>, string> type_a = get_type(tac[i][0], tac[i][3]);
                        vm.push_back("push temp "+ indx + " STR");
                        vm.push_back("pop "+type_a.second+" "+type_a.first.first+ " STR");
                ...
                if(tac[i][1] == "["){
                    ...
                    vm.push_back("push constant " + local[tac[i][2]].first + " INT");
                        ...
                        vm.push_back("push argument 2 INT");
                        vm.push_back("pop local 2 INT");
                        vm.push_back("push constant local 2  INT");
                        vm.push_back("push constant local 2  INT");
                        vm.push_back("push constant local 2  INT");
                        vm.push_back("push constant local 2  INT");
                        }
...
int main(int argc,char* argv[]){
    ...
    string s=argv[1];
    // cout<<argv[1];
     std::ofstream vmFile(s, std::ios::trunc);

    if (!vmFile) {
        std::cerr << "Error: Failed to open vm.vm file." << std::endl;
        return 1;
    }

    // Print VM instructions to the vm.vm file
    // for (const std::string& vmInstruction : vm) {
    //     vmFile << vmInstruction << std::endl;
    // }
        for(int i=0; i<vm.size(); i++){
        if(vm[i].substr(0,8) == "function"){
            vector<string> temp;
            temp = tokenize(vm[i]);
            vmFile << temp[0] + " " + temp[1] + " " + to_string(fun_var_count[temp[1]].first) + " " + to_string(fun_var_count[temp[1]].second) << " " << temp[2] << endl;
        }
        else
            vmFile << vm[i] << endl;
    }

    // Close the vm.vm file
    vmFile.close();
